...
> Concrete bad/good examples for implementing TradePulse services.
> For architectural patterns, see [Patterns.md](Patterns.md).
> For package layout, see [FolderStructure.md](FolderStructure.md).
> For quick project context, see [Architecture.md](../architecture/Architecture.md).
...
public final class RedisKeys {
    ...
    public static String rateLimit(UUID userId) {
        return "rate_limit:" + userId;
    }

    public static String portfolioValue(UUID userId) {
        return "portfolio_value:" + userId;
    }
...
#### **7. Kafka Consumer — Idempotency and Error Handling**
...
auditLogRepository.save(auditLog);  // UPDATE — forbidden
```
    ...
    public String readDatabasePassword() {
        GetSecretValueResponse response = secretsManagerClient.getSecretValue(
                GetSecretValueRequest.builder()
                    .secretId("tradepulse/auth-service/database")
                    .build()
        );
...
public class OrderService {
    public OrderResponse place(PlaceOrderRequest request) {
        ...
        log.info("Order placed: userId={}, symbol={}, side={}",
            request.userId(), request.symbol(), request.side());
