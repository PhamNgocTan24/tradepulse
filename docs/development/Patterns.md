...
> Architecture and distributed-systems patterns for TradePulse.
> For implementation-level examples, see [Syntax.md](Syntax.md).
> For service architecture, see [Architecture.md](../architecture/Architecture.md).
> For API contracts, see [ApiContracts.md](../api/ApiContracts.md).
...
public class PortfolioCommandService {
    ...
    public void applyFill(PortfolioEvent event) {
        ...
        redisTemplate.opsForValue().set(
            "portfolio_value:" + event.userId(),
            event.totalValue().toPlainString(),
...
public class BinanceTickerAdapter {
    public MarketTick toDomain(BinanceTickerResponse response) {
        return new MarketTick(
            response.symbol(),
            new BigDecimal(response.closePrice()),
...
// Spring Boot Actuator exposes /actuator/health/liveness and /actuator/health/readiness separately.
// application.yml:
